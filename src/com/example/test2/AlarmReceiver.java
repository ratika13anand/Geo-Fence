...
public class AlarmReceiver extends BroadcastReceiver implements
	...
	public void onReceive(Context context, Intent intent) {
		...
		sVibrate = sharedPref.getBoolean(SettingsActivity.KEY_PREF_VIBRATE,
				true);
	...
	protected void pingServerAtIntervals() {
		...
		if (lat != null && lng != null) {
			...
			try {
				URL url = new URL(
						"http://csci587team7.cloudapp.net:8080/587Service/rest/overlapfence/currentloc/"
								+ curloc);
	...
	private class getOverlapData extends AsyncTask<URL, Integer, String> {
		...
		protected void onPostExecute(String res) {
			if (res != null) {
				mNM = (NotificationManager) sContext
						.getSystemService("notification");
				// Display a notification about us starting. We put an icon in
				// the status bar.
	...
	private void showNotification(String res) {
		...
		CharSequence text = "Geo-Fences(s) found less than 25 yards away. View Details before proceeding.";
		...
		notification.setLatestEventInfo(sContext, "Geo-Fence Alert!", text,
				contentIntent);
